...
class ListWork(Command):
    ...
    def _output_header(self):
        print('{0:^30} | {1:^10} | {2:^45} | {3:^40}'.format('ID', 'Status', 'Title', 'Task'))
        print('{0:=<140}'.format(''))
    def _output(self, workitem=None):
        if workitem is None:
            return False
        print('{0:^30} | {1:^10} | {2:^45} | {3:^40}'.format(workitem.id, workitem.status, workitem.title, workitem.task.title))
        if len(workitem.timelog) > 0:
            for i in workitem.timelog:
                if i.stop is not None:
                    print('{0:>30} | {1:^10} | * {2:>20} / {3:>20} | {4}'.format('',
                                                                                 '',
                                                                                 convert_datetime_to_localtime(i.start).strftime('%Y-%m-%d %H:%M:%S'),
                                                                                 convert_datetime_to_localtime(i.stop).strftime('%Y-%m-%d %H:%M:%S'), ''))
                else:
                    print('{0:>30} | {1:^10} | * {2:>20} / {3:>20} | {4:40}'.format('',
                                                                                    '',
                                                                                    convert_datetime_to_localtime(i.start).strftime('%Y-%m-%d %H:%M:%S'),
                                                                                    'ongoing', ''))
        return True
